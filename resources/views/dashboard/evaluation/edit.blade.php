        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h3 class="text-2xl font-bold mb-6">Modifier l'évaluation de {{ $evaluation->student->name ?? '—' }}
                    </h3>
                            ...
                            <select name="academic_year_id" id="academic_year_id" required
                                ...
                                @foreach ($academicYears as $year)
                                    <option value="{{ $year->id }}"
                                        {{ old('academic_year_id', $evaluation->academic_year_id) == $year->id ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <select name="semester" id="semester" required
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    <option value="1"
                                        {{ old('semester', $evaluation->semester) == 1 ? 'selected' : '' }}>Semestre 1
                                    </option>
                                    <option value="2"
                                        {{ old('semester', $evaluation->semester) == 2 ? 'selected' : '' }}>Semestre 2
                                    </option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="note" id="note" step="0.01" min="0"
                                    max="20" value="{{ old('note', $evaluation->note) }}" required
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                        </div>
