        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h3 class="text-2xl font-bold mb-6">Modifier : {{ $module->name }} ({{ $module->code }})</h3>
                            ...
                            <select name="specialty_id" id="specialty_id" required
                                ...
                                @foreach ($specialties as $specialty)
                                    <option value="{{ $specialty->id }}"
                                        {{ old('specialty_id', $module->specialty_id) == $specialty->id ? 'selected' : '' }}>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="number" name="coefficient" id="coefficient"
                                    value="{{ old('coefficient', $module->coefficient) }}" min="1"
                                    max="20" required
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                            ...
                            <div>
                                ...
                                <input type="number" name="sequence" id="sequence"
                                    value="{{ old('sequence', $module->sequence) }}" min="1" max="99"
                                    required
                                    class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                            </div>
                        </div>
